...
{
    public class ListSummaryDAL
    {
        IDBOracleAdapter da = new IDBOracleAdapter();
        #region Open
        internal DataSet Open_LSTSUMRY()
        {
            ...
            try
            {
                ...
                if (ConfigurationManager.AppSettings["DataB"] == "SQL")
                {
                  ...
                  // string spCall = Helper.Ins.GetSPCall((dbManager.Parameters, "Open_Grid");
                    string spCall = Helper.Ins.GetSPCall(dbManager.Parameters, "Open_LSTSUMRY");
                    ds = dbManager.ExecuteDataSet(CommandType.StoredProcedure, "Open_LSTSUMRY");
