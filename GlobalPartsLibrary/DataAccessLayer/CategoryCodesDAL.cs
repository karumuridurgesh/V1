...
{
    public class CategoryCodesDAL
    {
        IDBOracleAdapter da = new IDBOracleAdapter();
        #region Open
        internal DataSet Open_GPMCATCDE()
        {
            ...
            try
            {
                ...
                if (ConfigurationManager.AppSettings["DataB"] == "SQL")
                {
                    ...
                    string spCall = Helper.Ins.GetSPCall(dbManager.Parameters, "Open_GPMCATCDE");
                    ds = dbManager.ExecuteDataSet(CommandType.StoredProcedure, "Open_GPMCATCDE");
        ...
        #endregion
        #region Save
        internal int Save_GPMCATCDE(string xmlData)
        {
            IDBManager dbManager = CommonConnection.Connectionstring();
            try
            {
                ...
                if (ConfigurationManager.AppSettings["DataB"] == "SQL")
                {
                    string spCall = Helper.Ins.GetSPCall(dbManager.Parameters, "Save_GPMCATCDE");
                    res = Convert.ToInt32(dbManager.ExecuteNonQuery(CommandType.StoredProcedure, "Save_GPMCATCDE"));
                }
                return res;
            }
            catch (Exception ex)
