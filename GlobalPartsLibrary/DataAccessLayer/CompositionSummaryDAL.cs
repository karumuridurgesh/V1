...
{
    public class CompositionSummaryDAL
    {
                IDBOracleAdapter da = new IDBOracleAdapter();
        #region Open
                internal DataSet Open_CMPSUMRY()
        {
            ...
            try
            {
                ...
                if (ConfigurationManager.AppSettings["DataB"] == "SQL")
                {
                  ...
                  // string spCall = Helper.Ins.GetSPCall((dbManager.Parameters, "Open_Grid");
                    string spCall = Helper.Ins.GetSPCall(dbManager.Parameters, "Open_CMPSUMRY");
                    ds = dbManager.ExecuteDataSet(CommandType.StoredProcedure, "Open_CMPSUMRY");
