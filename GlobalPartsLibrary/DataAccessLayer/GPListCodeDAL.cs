    ...
    public class GPListCodeDAL
    {
        ...
        internal DataSet Open_PagingGrid()
        {
            ...
            try
            {
                ...
                if (ConfigurationManager.AppSettings["DataB"] == "SQL")
                {
                    ...
                    string spCall = Helper.Ins.GetSPCall(dbManager.Parameters, "Open_GPLSTCD");
                    ds = dbManager.ExecuteDataSet(CommandType.StoredProcedure, "Open_GPLSTCD");
        ...
        public DataSet Fetch_GPLSTCD(string ListCode)
        {
            ...
            try
            {
                ...
                if (ConfigurationManager.AppSettings["DataB"] == "SQL")
                {
                    string spCall = Helper.Ins.GetSPCall(dbManager.Parameters, "Fetch_GPLSTCD");
                    ds = dbManager.ExecuteDataSet(CommandType.StoredProcedure, "Fetch_GPLSTCD");
        ...
        internal int Save_GPLSTCD(string xmlData, string Mode, string Active,string ListCode,string LName)
        {
            IDBManager dbManager = CommonConnection.Connectionstring();
            try
            {
                ...
                if (ConfigurationManager.AppSettings["DataB"] == "SQL")
                {
                    ...
                    dbManager.AddParameters(4, "@ListCode", ListCode, ParameterDirection.Input, 20);
                    dbManager.AddParameters(5, "@LName", LName, ParameterDirection.Input, 50);
                }
                int res = 0;
                if (ConfigurationManager.AppSettings["DataB"] == "SQL")
                {
                    string spCall = Helper.Ins.GetSPCall(dbManager.Parameters, "Save_GPLSTCD");
                    res = Convert.ToInt32(dbManager.ExecuteNonQuery(CommandType.StoredProcedure, "Save_GPLSTCD"));
                }
